...
export function AddSongForm({ singers }: AddSongFormProps) {
    ...
    async function onSubmit(data: FormType) {
        ...
        if (result.success) {
            toast("提交成功！", { description: result.message });
        } else {
            toast("提交失败", { description: result.message });
        }
    }
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)}>
                        <Controller
                            ...
                            render={({ field, fieldState }) => (
                                <Field>
                                    <FieldLabel htmlFor="songName">
                                        歌曲名称
                                    </FieldLabel>
                                    <Input {...field} id="songName" />
                            ...
                            render={({ field, fieldState }) => (
                                <div className="my-4 flex flex-wrap gap-2">
                                    {singers.map((singer) => {
                                        ...
                                        return (
                                            <motion.div
                                                className={`cursor-pointer rounded-full px-4 py-1.5 transition-colors select-none ${isChecked ? "bg-green-500" : "bg-accent"}`}
                                                onClick={() => {
                                                    if (isChecked) {
                                                        field.onChange(
                                                            field.value.filter(
